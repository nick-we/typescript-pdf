...
describe('MultiPage PDF Generation', () => {
                ...
                child: new Column({
                    children: [
                        new TextWidget(`Section ${i + 1}`, {
                            style: { fontSize: 16, fontWeight: 700, color: '#2c3e50' }
                        }),
                        new TextWidget(
    ...
    test('should generate PDF with headers and footers', async () => {
                ...
                child: new Row({
                    ...
                    children: [
                        new TextWidget('MultiPage Test Document', {
                            style: { fontSize: 14, fontWeight: 700, color: '#003e50' }
                        }),
        ...
        const footer = new Container({
            child: new TextWidget('© 2024 TypeScript PDF - MultiPage Widget Test', {
    ...
    test('should generate PDF with mixed content types', async () => {
                ...
                child: new Column({
                    children: [
                        new TextWidget('Mixed Content Report', {
                            style: { fontSize: 24, fontWeight: 700, color: '#2c3e50' }
                        }),
                        new TextWidget('Demonstrating various widget types across pages', {
        ...
        const multiPageWidget = new MultiPage({
            ...
            header: (pageNum, totalPages) =>
                new TextWidget(`Mixed Content Report - Page ${pageNum}/${totalPages}`, {
                    style: { fontSize: 12, fontWeight: 700, color: '#6c757d' }
                }),
            footer: new Container({
                child: new TextWidget('Generated with MultiPage Widget System', {
        ...
        const reportContent = [
                ...
                child: new Column({
                    children: [
                        new TextWidget('EXECUTIVE SUMMARY', {
                            style: { fontSize: 20, fontWeight: 700, color: '#2c3e50' }
                        }),
                        new TextWidget(
            ...
            ...Array.from({ length: 20 }, (_, i) =>
                new Container({
                    child: new Column({
                        children: [
                            new TextWidget(`Section ${i + 1}: Analysis Report`, {
                                style: { fontSize: 14, fontWeight: 700, color: '#34495e' }
                            }),
                            new TextWidget(
                            ...
                            new Row({
                                children: [
                                    new Container({
                                        child: new TextWidget('Status: Complete', {
                                            style: { fontSize: 10, color: '#ffffff', fontWeight: 700 }
                                        }),
                                        padding: Layout.EdgeInsets.symmetric({ horizontal: 8, vertical: 4 }),
                                        decoration: { color: '#27ae60' }
                                    }),
                                    new Container({
                                        child: new TextWidget(`Priority: ${i % 3 === 0 ? 'High' : i % 3 === 1 ? 'Medium' : 'Low'}`, {
                                            style: { fontSize: 10, color: '#ffffff', fontWeight: 700 }
                                        }),
                                        padding: Layout.EdgeInsets.symmetric({ horizontal: 8, vertical: 4 }),
                                        decoration: {
                                            color: i % 3 === 0 ? '#e74c3c' : i % 3 === 1 ? '#f39c12' : '#95a5a6'
                                        },
                                        margin: Layout.EdgeInsets.only({ left: 8 })
                                    })
                                ]
                            })
                        ]
    ...
    test('should generate performance test PDF with large content', async () => {
        console.log('🧪 Testing performance with large content PDF generation...');
                    ...
                    children: [
                        ...
                        ...(i % 10 === 0 ? [
                            new Table({
                                data: [
                                    ['Metric', 'Value', 'Unit'],
                                    ['Processing Time', `${(i * 0.1).toFixed(1)}`, 'ms'],
                                    ['Memory Usage', `${(45 + i * 0.5).toFixed(1)}`, 'MB'],
                                    ['Items Processed', `${i + 1}`, 'count']
                                ],
                                columnWidths: [
                                    { type: TableColumnWidthType.Flex, value: 2 },
                                    { type: TableColumnWidthType.Flex, value: 1 },
                ...
                new Container({
                    child: new Row({
                        ...
                        children: [
                            ...
                            new TextWidget(`${pageNum}/${totalPages} (${largeContent.length} items)`, {
