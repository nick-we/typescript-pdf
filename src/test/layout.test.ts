...
describe('Layout Systems', () => {
    ...
    describe('Box Constraints System', () => {
        ...
        it('should constrain sizes properly', () => {
            ...
            // Size too small
            const smallSize = Layout.BoxConstraints.constrain(constraints, { width: 20, height: 10 });
            expect(smallSize.width).toBe(50);
            expect(smallSize.height).toBe(30);

            // Size too large
            const largeSize = Layout.BoxConstraints.constrain(constraints, { width: 300, height: 200 });
            expect(largeSize.width).toBe(200);
            expect(largeSize.height).toBe(100);

            // Size within bounds
            const validSize = Layout.BoxConstraints.constrain(constraints, { width: 100, height: 60 });
        ...
        it('should validate constraint consistency', () => {
            ...
            expect(validConstraints.minWidth).toBeLessThanOrEqual(validConstraints.maxWidth);
            expect(validConstraints.minHeight).toBeLessThanOrEqual(validConstraints.maxHeight);
            ...
            expect(invalidConstraints.minWidth).toBeGreaterThan(invalidConstraints.maxWidth);
        });
    });
    ...
    describe('Edge Insets System', () => {
        ...
        it('should create symmetric edge insets', () => {
            const insets = Layout.EdgeInsets.symmetric({ horizontal: 20, vertical: 10 });
        ...
        it('should calculate horizontal and vertical totals', () => {
            const insets = Layout.EdgeInsets.only({ top: 5, right: 10, bottom: 15, left: 20 });
        ...
        it('should deflate size by insets', () => {
            ...
            const deflatedSize = Layout.EdgeInsets.deflateSize(insets, originalSize);
        ...
        it('should deflate constraints by insets', () => {
            const insets = Layout.EdgeInsets.symmetric({ horizontal: 10, vertical: 5 });
            const originalConstraints: Layout.BoxConstraints = {
                minWidth: 20, maxWidth: 100,
                minHeight: 15, maxHeight: 80
            };

            const deflatedConstraints = Layout.EdgeInsets.deflateConstraints(insets, originalConstraints);
    ...
    describe('Alignment System', () => {
        it('should resolve alignment positions correctly', () => {
            ...
            const centerPos = Layout.AlignmentUtils.resolve(
                Layout.Alignment.Center, containerSize, childSize
            );
            ...
            const topLeftPos = Layout.AlignmentUtils.resolve(
                Layout.Alignment.TopLeft, containerSize, childSize
            );
            ...
            const bottomRightPos = Layout.AlignmentUtils.resolve(
                Layout.Alignment.BottomRight, containerSize, childSize
            );
    ...
    describe('Flex Layout System', () => {
        ...
        it('should layout column with expanded children', () => {
            ...
            expect(layout.size.height).toBeLessThanOrEqual(mockLayoutContext.constraints.maxHeight);
        });
    ...
    describe('Constraint Solving', () => {
        it('should solve simple constraint propagation with padding', () => {
            ...
            const layout = container.layout(mockLayoutContext);
            const childLayout = new TextWidget('Content').layout(mockLayoutContext);
    ...
    describe('Coordinate Transformations', () => {
        it('should transform points correctly', () => {
            ...
            const matrix: Geometry.Matrix = { a: 2, b: 0, c: 0, d: 2, e: 5, f: 10 };
        ...
        it('should find rectangle intersections', () => {
            const rect1: Geometry.Rect = { x: 0, y: 0, width: 100, height: 100 };
